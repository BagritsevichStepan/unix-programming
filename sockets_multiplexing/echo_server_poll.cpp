...
namespace net {
    constexpr size_t kPollSize = 2048;
...
int main() {
    ...
    pollfd set[net::kPollSize];
    set[0].fd = server_socket;
    set[0].events = POLLIN;
    ...
    while (true) {
        ...
        const size_t set_size = index;

        if (poll(set, set_size, -1) == ERROR) {
            if (errno == EAGAIN) {
                continue;
            }
            net::utils::print_errno("Server: Failure during poll");
            break;
        }
        ...
        if ((set[0].revents & POLLIN) != 0 && net::accept_new_connection(server_socket, open_sockets) == ERROR) {
            break;
        }
    }
    return 0;
